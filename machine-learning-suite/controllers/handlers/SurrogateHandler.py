...
from    Handlers.AntennaMathHandler                        import  Matthew
...
class Golem:
    ...
    def trainAndMap(self, **kwargs) -> dict:
        ...
        if ( self.bShowOutput ):
            ...
            print("\t\t- Candidate: ", ApplicationHelper.round(self.lMap_Results[0], 2))
    ...
    def __train_srgHandler__(self, _eExit, _eGlobal, _eTR, _qTR) -> None:
        ...
        print("\t{" + ApplicationHelper.time() + "} - Starting surrogate training\t\t-> ", end="")
        ...
        for i in range(0, dArgs["max"]):
            ...
            fTmp_Fitness    = vSRG.train_set(cp.deepcopy(vData), advanced_training=False, compare=False)
    ...
    def __map_srgOverseer__(self, **kwargs) -> None:
        ...
        if (optimizer.bShowOutput):
            ...
            print("\t{" + ApplicationHelper.time() + "} - Starting surrogate mapping\t\t\t-> ", end="")
