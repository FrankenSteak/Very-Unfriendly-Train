...
class SwarmChan:
    ...
    def pso(self) -> None:
        ...
        for i in range(0, self.__iParticles):
            ...
            lAlpha1 = self.psoPhi1 * (np.array(lPBestTmp) - np.array(lPCurrTmp))
            lAlpha2 = self.psoPhi2 * (np.array(self.lBestSolution) - np.array(lPCurrTmp))
            ...
            self.lParticles[i].lCurrPosition = np.array(lPCurrTmp) + np.array(lBeta)
